...
-   Show how to convert those shapefiles into data frames that `ggplot2` can deal with
-   Introduce spatial geometries (`geom_`) of `ggplot2` and `tidyterra`.
...
Create maps with `geom_sf()` in a ggplot:
...
Check dimensions (only one depth and only one time step):
...
We could use this, but... we create a SpatVector object:
...
1.  Plot the `copernicus` raster data as a base map using `tidyterra::geom_spatraster()`.
2.  Add the `europe` vector data as a second layer (see Task 1). you can also use `tidyterra::geom_spatvector()`
...
Data processing and summary statistics, here only two very simple examples:
...
I manually changed separator from semicolon to comma and decimal from comma to point:

```{r, data1-odin}
odin <- read_csv("./data/iow/odin2__balticsea-ctd-oxygen-2023_comma.csv",
                 skip = 2) %>% 
...
## Select the lowest depth of each station
...
We need to convert the odin data frame to an sf object. **EPSG:4326** corresponds to the WGS 84 geographic coordinate system, which is based on latitude and longitude coordinates and widely used for mapping.

```{r, data4-odin}
odin_sf <- st_as_sf(odin.may.surf, coords = c("lon", "lat"), crs = 4326)
```
...
We need to convert the `codwgt` data frame to an sf object. **EPSG:4326** corresponds to the WGS 84 geographic coordinate system, which is based on latitude and longitude coordinates and widely used for mapping.
...
ggplot() +
  ...
  coord_sf(xlim = c(10, 22.3), ylim = c(53, 59.3)) +
  tidyterra::scale_fill_hypso_c(
    palette = "meyers_bathy",
    n.breaks = 12,
    guide = "none",
  ...
  geom_sf(data = codwgt_sf, 
          aes(geometry = geometry, col = CatchWgt),
          size = 4, alpha = 0.5) +
  scale_color_viridis()+
  labs(x = "Longitude",
       y = "Latitude",
...
We can use `ggplot2` to plot the `sf` data. Since these data are all data frames (sf features), we can use the `geom_sf()` function.
...
ggplot() + 
  geom_sf(data = baltic_drain,
          aes(fill = DESCRIPT),
          alpha = 0.7, color="grey30", lwd=0.4,
          show.legend = FALSE) +
...
We need to convert the cities data frame to an sf object. **EPSG:4326** corresponds to the WGS 84 geographic coordinate system, which is based on latitude and longitude coordinates and widely used for mapping.
...
ggplot() + 
  ...
  geom_sf(data = cities_sf, aes(geometry = geometry), color = "red", size = 3) +
  geom_sf_text(data = cities_sf, aes(label = city_name), nudge_y = 0.3, size = 3) +
  theme_minimal()
```
...
rostock.sat <- get_map(getbb("Rostock"), 
                   maptype='satellite', 
                   source="google", api_key=api_secret, 
                   zoom=12)
...
```{r, plot3-maps}
ggplot(baltic.countries.maps, aes(x = long, y = lat)) +
  geom_polygon(aes( group = group, fill = region))+
...
ggplot(baltic.countries.maps, 
  ...
  theme(legend.position = "none") +
  coord_fixed(1.3) +
  geom_point(data = cities, aes(x = long, y = lat), color = "black", size = 5)
```
